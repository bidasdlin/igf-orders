...
async function getAccessToken(): Promise<string> {
  ...
  if (!response.ok) { const error = await response.text(); throw new Error(`QB token refresh failed: ${error}`) }
...
async function qbRequest(method: string, endpoint: string, body?: object) {
  ...
  if (!response.ok) { const error = await response.text(); throw new Error(`QB API error [${response.status}]: ${error}`) }
...
export interface QBPurchaseOrder {
  ...
  Line: Array<{ DetailType: string; Amount: number; Description?: string }>
}

// Dynamically find the COGS account instead of hardcoding ID '1'
...
export async function createVendor(vendorName: string): Promise<QBVendor> {
  const data = await qbRequest('POST', 'vendor', { DisplayName: vendorName, CompanyName: vendorName, PrintOnCheckName: vendorName })
  return data.Vendor
}
...
export async function createPurchaseOrder(po: IGFPurchaseOrder): Promise<QBPurchaseOrder> {
  ...
  const lines = po.lineItems.map((item, index) => {
    ...
    return {
      ...
      AccountBasedExpenseLineDetail: { AccountRef: cogsAccount, BillableStatus: 'NotBillable', Qty: qty, UnitPrice: item.unitPrice },
      LineNum: index + 1,
    }
  })
  const notesLine = po.notes ? [{
    ...
    AccountBasedExpenseLineDetail: { AccountRef: cogsAccount, BillableStatus: 'NotBillable' },
  ...
  const body = {
    ...
    CustomField: [{ DefinitionId: '1', Name: 'Customer PO#', Type: 'StringType', StringValue: po.customerPONumber }],
...
export async function listPurchaseOrders(params?: { startDate?: string; endDate?: string; status?: 'Open' | 'Closed' }): Promise<QBPurchaseOrder[]> {
