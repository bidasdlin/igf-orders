...
function lineItemsMatchTotal(lineItems: Array<{ amount: number }>, totalAmount: number): boolean {
  if (lineItems.length === 0 || totalAmount <= 0) return false
  const lineTotal = lineItems.reduce((sum, item) => sum + (Number(item.amount) || 0), 0)
  return Math.abs(lineTotal - totalAmount) <= 0.05
}
...
function extractPOData(text: string, fileName: string) {
  ...
  const lineItems = (() => {
    if (multiLineItems.length === 0) return mergedItem ? [mergedItem] : []
    if (lineItemsMatchTotal(multiLineItems, totalAmount)) return multiLineItems

    // Some IGF PDFs split rows in a way that lets us recover only part of the
    // table as separate lines. Do not send a partial line-item total to QB.
    return mergedItem ? [mergedItem] : multiLineItems
  })()
